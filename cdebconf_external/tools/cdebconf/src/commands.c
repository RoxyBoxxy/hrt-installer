 ...
 */
char *
command_external(struct confmodule *mod, char *arg)
{
    char **argv;
    int argc;
    int count;
    ...
    argc = strcmdsplit(arg, argv, count);
    CHECKARGC(>= 1);

    rc = mod->frontend->methods.external(mod->frontend, argv);
    if (rc == 0)
        asprintf(&out, "%u OK", CMDSTATUS_SUCCESS);
    else
        asprintf(&out, "%u return code %d", CMDSTATUS_GOBACK, rc);
    return out;
}
