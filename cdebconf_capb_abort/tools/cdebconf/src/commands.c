...
command_capb(struct confmodule *mod, char *arg)
{
    ...
    for (i = 0; i < argc; i++) {
        if (strcmp(argv[i], "backup") == 0)
            ...
            mod->frontend->capability |= DCF_CAPB_ABORT;
    }

    asprintf(&out, "%u multiselect backup abort", CMDSTATUS_SUCCESS);
    return out;
}
...
command_go(struct confmodule *mod, char *arg)
{
    ...
    if (mod->frontend->methods.go(mod->frontend) == CMDSTATUS_GOBACK)
    {
        ...
        mod->frontend->methods.update_seen_questions(mod->frontend, STACK_SEEN_REMOVE);
    }
    else
    {
        asprintf(&out, "%u ok", CMDSTATUS_SUCCESS);
        mod->frontend->methods.update_seen_questions(mod->frontend, STACK_SEEN_ADD);
    }
    mod->frontend->methods.clear(mod->frontend);
